                                                     ...
                                                     const std::string& mode) // Возвращает std::shared_ptr<FileHandle>
{
    ...
    auto it = cache_.find(path);// ищет в таблице элемент с ключом path и возвращает итератор - который указывает на пару ключ-значение внутри контейнера
    ...
    auto newFile = std::make_shared<FileHandle>(path, mode);// создаем новый FileHandle (make выделяет память сразу на блок)
...
{
    for (auto it = cache_.begin(); it != cache_.end();)//возвращаемся к первому элементу кэша и проверяем не дощли ли до конца
    {
